...
;; 2023-0130: doom's web/yasnippets module cause emmet to be active everywhere
;; - this is supposed to happen when loading auto-yasnippet, but it is deferred
;; - the bug seems inconsistent, but it essentially redefines <tab> everywhere to
;;   expand emmet code on ... <fucking>everything</fucking>
;; - at some point, the state in emacs goes bad and yas-global-mode triggers
;;   emmet-mode everywhere (as far as i can see)
;;   - because the effect is global, it poisons the state in all buffers and they all
;;     now have these hooks.
;;   - this may not be correct though. but i have to desktop-save and reload the
;;     session when the state is poisoned.
;; - i finally dug into advice-add. i knew this would be more helpful than I
;;   realized, but never seemed to have enough time to do so. It helps to
;;   talk to someone who uses emacs. the defadvice, advice.el and defadvice!
;;   source is esp. helpful for macro-writing.
...
(dc/toggleable-boolean native-comp-async-report-warnings-errors "C-c t N")
