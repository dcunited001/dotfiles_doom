...
This is too much for me. Also I have a bug where editing a buffer name makes the
UI element unclickable ... so you can never close it. IDK. The config is still
...
I'm transitioning to dired and consistently killing all my dired buffers with
=q= ... I'm remapping this for now.
...
This configures org-roam v2 and is not compatible with doom's org-roa module. See the updated Hitchiker's Guide for more info. As long as one bears in mind the differences between v1 and v2, then these links are also useful
...
Add Content from [[github:daviwil/dotfiles][daviwil/dotfiles]]
...
Fix the Org Roam keymaps that doom provides
...
**** TODO setup =org-roam-protocol= and mime types for org roam
...
+ [ ] HaoZeke [[file:/data/ecto/x.files/HaoZeke/doom/config.org::*Templates][protocol links]]
...
**** TODO configure =org-refile-targets= in the projectile template for =.dir-locals.el=

+ The following works for /refiling out of projects with todo.org/ or for
  cleaning up projects with messy todo lists mixed into org notes
  - =(("./todo.org" "./notes.org") . (:maxlevel . 3))=
  - it's convenient because you can initialize =org-agenda-files= to something
    sparse when emacs starts, but still manage to process errant TODO's in org
    files ... without adding/removing files to =org-agenda-files=
    - it keeps your =org-refile-targets= slim while generally adding what you
      want when you want it... ?
  - however, it needs to be set up in =.dir-locals.el= and it needs to be
    configured like =(locate-dominating-file default-directory "todo.org")=
